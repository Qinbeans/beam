...
namespace beam {
class ValueBox : public GameObject {
protected:
  std::string text;
  Vector2 position;
  Vector2 size;
  ...
  std::function<void(ValueBox *, SharedManager)> changeCallback;

public:
  ValueBox(const std::string &text, Vector2 position, Vector2 size,
  ...
  void onChange(std::function<void(ValueBox *, SharedManager)> callback);
};
} // namespace beam
