...
namespace beam {
class ToggleSlider : public GameObject {
protected:
  ...
  std::function<void(ToggleSlider *, SharedManager)> changeCallback;

public:
  ToggleSlider(const std::vector<std::string> &items, Vector2 position,
  ...
  void onChange(std::function<void(ToggleSlider *, SharedManager)> callback);
};
} // namespace beam
