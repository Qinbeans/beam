...
protected:
  ...
  std::function<void(Slider *, SharedManager)> changeCallback;

public:
  Slider(Vector2 position, Vector2 size, float value = 0.0f,
         float minValue = 0.0f, float maxValue = 1.0f, Color bg = LIGHTGRAY,
         Color fg = DARKGRAY, const std::string &textLeft = "",
  ...
  void onChange(std::function<void(Slider *, SharedManager)> callback);
};
} // namespace beam
